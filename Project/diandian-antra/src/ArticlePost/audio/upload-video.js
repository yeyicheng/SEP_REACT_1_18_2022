    ...
    uploadVideo(files) {
        ...
        files.forEach(file => {
            ...
            if (
                ...
                // 后缀名不合法，不是视频
                errInfos.push(`【${name}】${t('Invalid format')}`)
                return
            }

            if (uploadVideoMaxSize < size) {
                // 上传视频过大
                errInfos.push(`【${name}】${t('exceeds')} ${uploadVideoMaxSize}M`)
                return
            }
            //验证通过的加入结果列表
        ...
        if (uploadVideoServer) {
            ...
            uploadVideoParams.forEach((key, val) => {
     ...
     insertVideo(url) {
        ...
        if (!config.customInsertVideo) {
            if (UA.isFirefox) {
                editor.cmd.do(
                    'insertHTML',
                    `<p data-we-video-p="true"><audio controls="controls" style="max-width:100%"><source src="${url}" /></audio></p><p>&#8203</p>`
                )
            } else {
                editor.cmd.do(
                    'insertHTML',
                    `<audio controls="controls" style="max-width:100%"><source src="${url}" /></audio>${EMPTY_P}`
                )
            }
        } else {
